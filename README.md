...
| 连续对话 | 服务端按会话返回继续监听后，设备使用本地 VAD；检测到约 2 秒静音自动提交下一轮，按左键或 BOOT 可退出。 |
| 语音唤醒 | RLCD 4.2 固件支持“你好奥拉”和“你好欧拉”唤醒，并保留声学门控以减少误触发。 |
| 电量显示 | RLCD 4.2 从板载电池 ADC 读取电压，在状态栏显示百分比；无法读到有效电压时显示未知状态。 |
...
当前公开版固件构建已包含连续对话、语音唤醒和 RLCD 电量显示；连续对话由设备每轮 `start` 帧的 `continuous_dialogue` 会话字段控制，未携带该字段的旧客户端仍保持按键结束录音的行为。
